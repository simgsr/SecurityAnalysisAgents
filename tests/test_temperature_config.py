...
from securityanalysisagents.llm_clients.factory import create_llm_client
...
class TestTemperatureEnvOverlay:
    def test_env_sets_temperature(self, monkeypatch):
        import securityanalysisagents.default_config as dc
        monkeypatch.setenv("SECURITYANALYSISAGENTS_TEMPERATURE", "0.2")
        importlib.reload(dc)
        ...
        monkeypatch.delenv("SECURITYANALYSISAGENTS_TEMPERATURE", raising=False)
        importlib.reload(dc)
...
class TestProviderKwargsTemperature:
    ...
    def _kwargs_for(self, temperature):
        ...
        graph = SecurityAnalysisAgentsGraph.__new__(SecurityAnalysisAgentsGraph)
        graph.config = {"llm_provider": "openai", "temperature": temperature}
        return SecurityAnalysisAgentsGraph._get_provider_kwargs(graph)
