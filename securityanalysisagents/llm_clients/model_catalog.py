...
MODEL_OPTIONS: ProviderModeOptions = {
    ...
    # Ollama: the interactive CLI lists whatever the running server actually
    # has — cli.utils.select_ollama_model() queries the live /v1/models
    # endpoint so the menu mirrors `ollama list` instead of a hardcoded set
    # that goes stale. This static entry is only the fallback shown when the
    # server can't be reached (and the source for get_known_models): the two
    # DeepSeek ``:cloud`` defaults plus "Custom model ID". Those cloud models
    # are proxied by a local ollama-serve (keyless from here; the host must be
    # signed in via `ollama signin`) and may not appear in a local pull list.
    # Labels omit a "local" marker since the endpoint is configurable via
    # OLLAMA_BASE_URL and the resolved URL is shown by confirm_ollama_endpoint().
    "ollama": {
        "quick": [
            ("DeepSeek V4 Flash (cloud) - Fast, thinking + non-thinking", "deepseek-v4-flash:cloud"),
            ("Custom model ID", "custom"),
        ],
        "deep": [
            ("DeepSeek V4 Pro (cloud) - Latest flagship", "deepseek-v4-pro:cloud"),
            ("DeepSeek V4 Flash (cloud) - Fast, supports thinking", "deepseek-v4-flash:cloud"),
            ("Custom model ID", "custom"),
        ],
    },
